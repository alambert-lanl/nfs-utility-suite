...
impl ValidatedStruct {
    ...
    pub(super) fn deserialize_definition_zcopy(
        ...
        buf.code_block(
            "fn validate(self) -> Result<Self, xdr_lib::DeserializeError>",
            |buf| {
        ...
        for dep in deps.iter() {
            buf.code_block(
                &format!(
                    "pub fn get_{}_width(&self) -> Result<usize, xdr_lib::DeserializeError>",
                    dep
                ),
                |buf| {
        ...
        for (member, member_off) in self.members.iter() {
            buf.code_block(
                ...
                |buf| {
                    if member.size(tab, size_tab).is_none() {
                        if member.is_varlen_reader(tab, size_tab) {
                            buf.add_line(&format!(
                                "return self.{}.get().unwrap().clone()",
                                member.name
                            ));
                            return;
                        } else if member.is_varlen_optional_reader(tab, size_tab) {
                            buf.add_line(&format!(
                                "return self.{}.get().map(|v| v.clone());",
                                member.name
                            ));
                            return;
                        }
                    ...
                    member.deserialize_inline_zcopy(buf, tab, size_tab);
                },
            );
        }
    }
...
impl ValidatedEnum {
    ...
    pub(super) fn deserialize_definition_zcopy(
        ...
        buf.code_block(
            "pub fn deserialize(_input: &[u8]) -> Result<Self, xdr_lib::DeserializeError>",
            |buf| {
                buf.add_line("let val = xdr_lib::get_i32_immut(_input);");
...
impl XdrType {
    ...
    pub(super) fn deserialize_inline_zcopy(
        ...
        if let XdrType::Name(name) = self {
            let definition = tab.lookup_definition(name);
            if let ValidatedDefinition::TypeDef(ref tdef) = *definition {
                tdef.decl.deserialize_inline_zcopy(buf, tab, size_tab);
                return;
            };
        };
    ...
    pub(super) fn get_size_inline_zcopy(
        ...
        if let XdrType::Name(name) = self {
            ...
            if let ValidatedDefinition::TypeDef(ref tdef) = *definition {
                tdef.decl
                    .get_size_inline_zcopy(buf, tab, size_tab, fallible_parent, member_name);
                return;
            };
        };
        ...
        if let Some(found_size) = self.size(size_tab) {
    ...
    fn deserialize_method_zcopy(&self, tab: &ValidatedSymbolTable) -> (String, bool) {
        match self {
            XdrType::Int => ("xdr_lib::get_i32_immut".to_string(), false),
            XdrType::UInt => ("xdr_lib::get_u32_immut".to_string(), false),
            XdrType::Hyper => ("xdr_lib::get_i64_immut".to_string(), false),
            XdrType::UHyper => ("xdr_lib::get_u64_immut".to_string(), false),
            XdrType::Float => todo!(),
            XdrType::Double => todo!(),
            XdrType::Quadruple => todo!(),
            XdrType::Bool => ("xdr_lib::get_bool_immut".to_string(), false),
            XdrType::Name(n) => {
